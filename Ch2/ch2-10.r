...
x.cv <- x[-l,]
y.cv <- as.vector(y[-l])
fit <- flare.slim(X=x.cv,Y=y.cv,method="dantzig",lambda=lam) #Dantzig selector推定量
b.est <- fit$beta #回帰係数の推定値
pred <- x[l,]%*%b.est #目的変数の予測
...
#最終的なDantzig selector推定量
fit <- flare.slim(X=x.cv,Y=y.cv,method="dantzig",lambda=opt.lambda)
print(fit$beta)　#最終的なDantzig selector推定量の表示
